    ...
    while (ros::ok() && (field_max<5000))
    {
        ...
        *last_cloud = *cloud;
        ...
        if (have_last_cloud) {
            ...
            best_pt = max_pt;
            //std::cout << "max pt idx" << max_index << " (" << field_max << "):" << max_pt.x() << " " << max_pt.y() << " " << max_pt.z() << std::endl;
        } else {
...
{
        led_k = locate_led();
        tf::Stamped<tf::Pose> led_tf_pose = getPose(rgb_optical_frame_.c_str(),"r_gripper_led_frame");
...
int main(int argc,char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        double radius = .2;
        lookAt("r_gripper_led_frame", radius * sin(angle), radius * cos(angle),0);
        ...
        find_correspondance(led_tf, led_kinect);
        ...
        if (points_in_tf.points.size() > 2) {
            ...
            transformation_estimator.estimateRigidTransformation(points_in_tf, points_in_kinect, trans);
            ...
            std::cout << "kinect pose post " << ps_post << std::endl;
